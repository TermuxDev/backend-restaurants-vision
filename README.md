...
## Deploiement sur Render (Uvicorn)

1. Ajoutez un service `Web Service` dans Render.
2. Dans `Build Command` :

```bash
pip install -r requirements.txt
```

3. Dans `Start Command` :

```bash
uvicorn config.asgi:application --host 0.0.0.0 --port $PORT --env-file .env
```

4. Variables d'environnement importantes sur Render :

- `DJANGO_SECRET_KEY` (clé secrète forte)
- `DJANGO_DEBUG=False`
- `DJANGO_ALLOWED_HOSTS=*` ou domaine Render
- `MYSQL_DB`, `MYSQL_USER`, `MYSQL_PASSWORD`, `MYSQL_HOST`, `MYSQL_PORT` (si vous utilisez MySQL)

5. Après déploiement, faites les migrations :

- Via `Render Shell` ou commande de build :
...
6. Vérifiez le point de terminaison docs :
